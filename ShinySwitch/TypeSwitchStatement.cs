...
namespace ShinySwitch
{
    public class TypeSwitchStatement<TSubject> : SwitchStatement<TSubject>
    {
        ...
        public TypeSwitchStatement<TSubject> Match<T>(Action<T> action) where T : TSubject => MatchInternal(x => subject is T, action);
        public TypeSwitchStatement<TSubject> Match<T>(Func<T, bool> predicate, Action<T> action) where T : TSubject => MatchInternal(x => subject is T && predicate((T) x), action);
        ...
        public TypeSwitchStatement<TSubject> Then(Action<TSubject> action) => MatchInternal(x => result.HasResult, action);

        public TypeSwitchStatement<TSubject> MatchInternal<T>(Func<TSubject, bool> predicate, Action<T> action) where T:TSubject
        {
